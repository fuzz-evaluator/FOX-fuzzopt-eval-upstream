...
# XXX: Needed to build fuzz_target in bloaty
...
ninja -j$(nproc)
cp fuzz_target ../binaries/aflpp_build 
...
cmake -G Ninja -DBUILD_TESTING=false .
ninja -j$(nproc)
cp fuzz_target ../binaries/cmplog_build
...
cmake -G Ninja -DBUILD_TESTING=false .
ninja -j$(nproc)
cp fuzz_target ../binaries/optfuzz_build
